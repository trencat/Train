...
func (f *Factory) GetTrain() core.Train {
	return core.Train{
		ID:            f.rand.Int(),
		...
		MaxTraction:   2e5 + 4e5*f.rand.Float64(),            // Range [2e5, 6e5]
		MaxBrake:      2e5 + 4e5*f.rand.Float64(),            // Range [2e5, 6e5]
		MaxVelocity:   90 + 30*f.rand.Float64(),              // Range [60, 120]
...
func (f *Factory) GetTrack(number int, minLength float64, maxLength float64,
	slope bool, bend bool, tunnel bool) ([]core.Track, error) {
	...
	railroad := make([]core.Track, number)
